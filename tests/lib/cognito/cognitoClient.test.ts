...
import {
	...
	AdminRespondToAuthChallengeCommand, GetUserCommand, DescribeUserPoolClientCommand,
...
describe('Get User from Token', () => {
	...
	test('as expected', async() => {
		...
		//Cognito called
		expect(cognitoIdpClientMock).toHaveReceivedCommandTimes(GetUserCommand, 1);
...
describe('User Login', () => {
	...
	test('as expected', async() => {
		...
		//Cognito called
		expect(cognitoIdpClientMock).toHaveReceivedCommandTimes(AdminInitiateAuthCommand, 1);
		let params = getMockAwsClientCommandParams(cognitoIdpClientMock, AdminInitiateAuthCommand);
...
describe('Initiate Custom Auth', () => {
	...
	beforeEach(() => {
		...
		cognitoIdpClientMock.on(AdminInitiateAuthCommand).resolves(initiateAuthResult);
	})
	...
	test('as expected', async () => {
		...
		expect(cognitoIdpClientMock).toHaveReceivedCommandTimes(AdminRespondToAuthChallengeCommand, 1);
		params = getMockAwsClientCommandParams(cognitoIdpClientMock, AdminRespondToAuthChallengeCommand);
		expect(params.UserPoolId).toEqual(userPoolId);
		expect(params.ClientId).toEqual(clientId);
...
describe('Respond to Auth Challenge', () => {
	...
	test('as expected', async() => {
		...
		//Cognito called
		expect(cognitoIdpClientMock).toHaveReceivedCommandTimes(AdminRespondToAuthChallengeCommand, 1);
		let params = getMockAwsClientCommandParams(cognitoIdpClientMock, AdminRespondToAuthChallengeCommand);
		expect(params.UserPoolId).toEqual(userPoolId);
		expect(params.ClientId).toEqual(clientId);
...
describe('Verify Client', () => {
	...
	beforeEach(() => {
		clientResponse = {
			UserPoolClient: {
				CallbackURLs: [
					"https://wwww.other.domain.com",
					uri
				]
			}
		};
		cognitoIdpClientMock.on(DescribeUserPoolClientCommand).resolves(clientResponse);
	})
	...
	test('invalid clientId', async () => {
		cognitoIdpClientMock.on(DescribeUserPoolClientCommand).rejects(new Error());
		let r = await client.verifyClient(clientId, uri);
		expect(r.valid).toBeFalsy();
