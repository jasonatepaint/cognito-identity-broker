...
describe("Set Grant", () => {
    test("as expected", async () => {
        ...
        expect(dbGrant.ttl).toEqual(
            ttlFromMinutes(GRANT_CONSTANTS.TTL_MINUTES),
        );
    ...
    test("missing attributes", async () => {
        await expect(
            setGrant(<any>undefined, redirectUri, credentials, codeChallenge),
        ).rejects.toThrowError();
        await expect(
            setGrant(clientId, <any>undefined, credentials, codeChallenge),
        ).rejects.toThrowError();
        await expect(
            setGrant(clientId, redirectUri, <any>undefined, codeChallenge),
        ).rejects.toThrowError();
    });
});
...
describe("Get Credentials from Grant", () => {
    ...
    test("as expected", async () => {
        const actual = await getCredentialsFromGrant(
            code,
            clientId,
            redirectUri,
            codeVerifier,
    ...
    test("expired code", async () => {
        ...
        const dt = new Date(
            dayjs.utc().add(GRANT_CONSTANTS.TTL_MINUTES, "minute").toDate(),
        );
        jest.useFakeTimers().setSystemTime(dt.getTime());

        await expect(
            getCredentialsFromGrant(code, clientId, redirectUri, codeVerifier),
        ).rejects.toThrowError();
    ...
    test("codeVerifier failure", async () => {
        ...
        await expect(
            getCredentialsFromGrant(code, clientId, redirectUri, undefined),
        ).rejects.toThrowError();
        await expect(
            getCredentialsFromGrant(code, clientId, redirectUri, "invalid"),
        ).rejects.toThrowError();
    });
