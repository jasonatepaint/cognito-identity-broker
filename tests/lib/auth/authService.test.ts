...
import {
    setGrant,
    getCredentialsFromGrant,
} from "../../../src/lib/auth/grants";
import jwt_decode from "jwt-decode";
import { buildFailedOAuth2AuthorizeResponse } from "../../../src/lib/auth/utils";
...
describe("Authorize Client", () => {
    ...
    beforeEach(() => {
        ...
        mockCognitoClient.prototype.initiateCustomAuth.mockResolvedValue(
    ...
    test("as expected", async () => {
        ...
        expect(
            CognitoClient.prototype.initiateCustomAuth,
        ).toHaveBeenCalledTimes(1);
        expect(CognitoClient.prototype.initiateCustomAuth).toHaveBeenCalledWith(
            clientId,
            username,
            accessToken,
        );
        ...
        const credentials = formatTokenResponse(
            successfulLoginResult.AuthenticationResult,
        );
    ...
    test("missing clientId", async () => {
        ...
        const expected = buildFailedOAuth2AuthorizeResponse(
    ...
    test("invalid client", async () => {
        ...
        mockCognitoClient.prototype.verifyClient.mockResolvedValue({
            valid: false,
            message,
        });
        const r = await svc.authorizeClient(parameters);
    ...
    test("initiate custom auth error", async () => {
        mockCognitoClient.prototype.initiateCustomAuth.mockRejectedValue(
            new Error(),
        );
        const r = await svc.authorizeClient(parameters);
        expect(r.success).toBeFalsy();
    ...
    test("initiate custom auth - UserLambdaValidationException", async () => {
        const error = new UserLambdaValidationException(<any>{
            message: "Access Token as expired",
        });
...
describe("Get Tokens for Client", () => {
    beforeEach(() => {
        ...
        mockGetCredentialsFromGrant.mockResolvedValue(
            successfulLoginResult.AuthenticationResult,
        );
        mockCognitoClient.prototype.refreshToken.mockResolvedValue(
            successfulLoginResult,
        );
    });
    ...
    test("Authorization Code Grant Type - as expected", async () => {
        ...
        expect(getCredentialsFromGrant).toHaveBeenCalledWith(
            code,
            clientId,
            redirectUri,
            codeVerifier,
    ...
    test("Refresh Token Grant Type - as expected", async () => {
        ...
        expect(CognitoClient.prototype.refreshToken).toHaveBeenCalledWith(
            clientId,
...
describe("Login User", () => {
    ...
    test("password - as expected", async () => {
        mockCognitoClient.prototype.userLogin.mockResolvedValue(
            successfulLoginResult,
        );
        ...
        expect(CognitoClient.prototype.userLogin).toHaveBeenCalledWith(
            clientId,
            username,
            password,
        );
    ...
    test("missing username", async () => {
        ...
        await expect(
            svc.login(clientId, loginParameters),
...
describe("Verify Auth Challenge", () => {
    ...
    beforeEach(async () => {
        ...
        mockCognitoClient.prototype.getUserFromToken.mockResolvedValue(
            cognitoUser,
        );
    });
    ...
    test("as expected", async () => {
        ...
        expect(CognitoClient.prototype.getUserFromToken).toHaveBeenCalledTimes(
            1,
        );
        expect(CognitoClient.prototype.getUserFromToken).toHaveBeenCalledWith(
            accessToken,
        );
    });
    ...
    test("handles exception", async () => {
        mockCognitoClient.prototype.getUserFromToken.mockRejectedValue(
            new Error(),
        );
