...
describe("Login", () => {
    ...
    test("password as expected", async () => {
        ...
        expect(AuthService.prototype.login).toHaveBeenCalledWith(req.clientId, {
...
describe("Client Authorize", () => {
    ...
    beforeEach(() => {
        ...
        mockAuthService.prototype.authorizeClient.mockResolvedValue({
    ...
    test("service exception", async () => {
        mockAuthService.prototype.authorizeClient.mockRejectedValue(
            new Error(),
        );
        const res = await clientOAuth2Authorize(event, context);
        const result = JSON.parse(<string>res.body);
...
describe("Client Tokens", () => {
    ...
    test("as expected", async () => {
        ...
        expect(AuthService.prototype.getTokensForClient).toHaveBeenCalledTimes(
            1,
        );
        expect(AuthService.prototype.getTokensForClient).toHaveBeenCalledWith(
            req,
        );
    });

    test("service exception", async () => {
        mockAuthService.prototype.getTokensForClient.mockRejectedValue(
            new Error(),
        );
        const res = await clientOAuth2Token(event, context);
        const result = JSON.parse(<string>res.body);
...
describe("Build Auth Cookies Header", () => {
    ...
    test("as expected", async () => {
        ...
        expect(cookies[0]).toEqual(
            cookie.serialize("accessToken", credentials.accessToken, options),
        );
        expect(cookies[1]).toEqual(
            cookie.serialize("idToken", credentials.idToken, options),
        );
        ...
        const maxAge =
            ...
            60 *
            24 *
            parseInt(process.env.COGNITO_REFRESH_TOKEN_EXPIRES_DAYS || "1");
        expect(cookies[2]).toEqual(
            cookie.serialize("refreshToken", credentials.refreshToken, {
                ...options,
                maxAge,
            }),
        );
