import {
	cognitoAuthTriggers,
} from "../../src/api/handlers/cognito-triggers-handler"
...
describe('Cognito Auth Triggers', () => {
	...
	describe('Create Auth Challenge', () => {
		test('as expected', async () => {
			...
			const res = <CreateAuthChallengeTriggerEvent>await cognitoAuthTriggers(event);
			expect(res.response.publicChallengeParameters.question).toEqual("TheQuestion");
			expect(res.response.privateChallengeParameters.answer).toEqual("TheAnswer");
	...
	describe('Define Auth Challenge', () => {
		test('no session', async () => {
			...
			const res = <DefineAuthChallengeTriggerEvent> await cognitoAuthTriggers(event);
		...
		test('invalid session', async () => {
			let event = <DefineAuthChallengeTriggerEvent>{
				triggerSource: "DefineAuthChallenge_Authentication",
				request: {
					session: [ { challengeName: "CUSTOM_CHALLENGE", challengeResult: false } ] //challengeRule false
				},
				response: {}
			};
	...
	describe('Verify Auth Challenge', () => {
		...
		beforeEach(() => {
			...
			mockAuthService.prototype.verifyAuthChallenge.mockResolvedValue(true);
		});
		...
		test('as expected', async () => {
			...
			const res = <VerifyAuthChallengeResponseTriggerEvent> await cognitoAuthTriggers(event);
			expect(res.response.answerCorrect).toBeTruthy();

			expect(AuthService.prototype.verifyAuthChallenge).toHaveBeenCalledTimes(1)
			expect(AuthService.prototype.verifyAuthChallenge).toHaveBeenCalledWith(event.userName, accessToken);
		});

		test('verify fails', async () => {
			mockAuthService.prototype.verifyAuthChallenge.mockResolvedValue(false);
			const event = <VerifyAuthChallengeResponseTriggerEvent>{
				userName: "123-123-123",
				triggerSource: "VerifyAuthChallengeResponse_Authentication",
				request: { challengeAnswer: accessToken },
