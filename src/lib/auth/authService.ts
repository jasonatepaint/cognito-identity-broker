...
export class AuthService {
	...
	async authorizeClient(parameters: oAuth2AuthorizeParameters): Promise<CodeFlowResponse | FailedLoginResponse> {
		const {clientId, redirectUri, state, codeChallenge, cookies} = parameters;

		const failedResponse = (reason: string) => {
			return buildFailedOAuth2AuthorizeResponse(reason, clientId, redirectUri, state);
		};
		...
		const hasTokens = !!(cookies?.idToken && cookies?.accessToken && cookies?.refreshToken);
		if (!hasTokens) {
		...
		try {
			...
			return buildLoginResponse<CodeFlowResponse>(true, AuthConstants.LoginResults.CodeFlowInitiated, {
				code,
				redirectUri: redirectUri + `?code=${code}${insertStateIfAny(state)}`,
				state
			});
		} catch (e) {
			console.error(e);
	...
	async getTokensForClient(parameters: oAuth2TokenParameters): Promise<UserLoginResponse> {
		...
		if (grantType === AuthConstants.ClientGrantTypes.Code) {
			try {
				const authentication = await getCredentialsFromGrant(code, clientId, redirectUri, codeVerifier);
				return buildLoginResponse<UserLoginResponse>(true, AuthConstants.LoginResults.LoggedIn, {authentication})
			} catch (e) {
				console.error(e);
	...
	async login(clientId: string, parameters: LoginParameters): Promise<UserLoginResponse> {
		...
		const result = await this.cognito.userLogin(clientId, username, password);
	 ...
	 */
	async refreshToken(clientId: string, refreshToken?: string): Promise<UserLoginResponse> {
		if (!refreshToken) {
			throw new Error("Missing Refresh Token");
		}
		const result = await this.cognito.refreshToken(clientId, refreshToken);
		const authentication = formatTokenResponse(result.AuthenticationResult);
		return buildLoginResponse<UserLoginResponse>(true, AuthConstants.LoginResults.Refreshed, {authentication});
	}
	...
	async verifyAuthChallenge(username: string, accessToken: string): Promise<boolean> {
		try {
			...
			console.warn("Unable to verify Auth Challenge for username: " + username, e);
		}
		return false;
	}
}
