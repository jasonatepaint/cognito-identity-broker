...
export const handleRequest = async <Response = any>(
    ...
    try {
        ...
        const data = options.resultHasHeaders
            ? (result as { data: any }).data
            : result;
        const headers = options.resultHasHeaders
            ? (result as { headers: any }).headers
            : {};
        response = options.buildResponse(
            ctx,
            options.successfulStatusCode,
            data,
            headers,
        );
    } catch (e) {
