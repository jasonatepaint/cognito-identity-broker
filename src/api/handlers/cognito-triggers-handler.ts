 ...
 */
export const cognitoAuthTriggers = async (event: CreateAuthChallengeTriggerEvent| DefineAuthChallengeTriggerEvent | VerifyAuthChallengeResponseTriggerEvent) => {
...
const createAuthChallenge = async (event: CreateAuthChallengeTriggerEvent) => {
    ...
    console.log("RESULT RESPONSE EVENT", JSON.stringify(event.response, null, 2));
    return event;
};
...
const defineAuthChallenge = async (event: DefineAuthChallengeTriggerEvent) => {
    ...
    if (event.request.session.length === 0) {
        ...
        if (session.challengeName === CUSTOM_CHALLENGE && session.challengeResult) {
...
const verifyAuthChallenge = async (event: VerifyAuthChallengeResponseTriggerEvent) => {
    const svc = new AuthService();
    const accessToken = event.request.challengeAnswer;
    event.response.answerCorrect = await svc.verifyAuthChallenge(event.userName, accessToken);
    console.log("RESULT RESPONSE EVENT", JSON.stringify(event.response, null, 2));
    return event;
};
