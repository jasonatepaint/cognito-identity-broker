...
export const clientOAuth2Authorize = async (
    ...
    return handleRequest(ctx, fn(), <HandleRequestOptions>{
        successfulStatusCode: 302,
        resultHasHeaders: true,
    });
};

export const clientOAuth2Token = async (
    event: APIGatewayProxyEvent,
    context: Context,
) => {
    console.log("EVENT", event);
 ...
 */
export const buildAuthCookiesHeader = (
    auth: oAuthTokenCollection,
    event: APIGatewayProxyEvent,
) => {
    ...
    if (tokens.accessToken) {
        cookies.push(
            cookie.serialize("accessToken", tokens.accessToken, options),
        );
    }
    if (tokens.idToken) {
    ...
    if (tokens.refreshToken) {
        const expiresDays = parseInt(
            process.env.COGNITO_REFRESH_TOKEN_EXPIRES_DAYS || "1",
        );
...
const handleAuthFlow = async (
    ...
    const fn = async () => {
        ...
        const headers = data.success
            ? buildAuthCookiesHeader(data.authentication, event)
            : {};
        return {
            headers,
            data,
